...
| `gpt-oss-120b-vllm.yaml` | `openai/gpt-oss-120b` | vLLM | LLM-judge batch backend. Harmony tool-call + reasoning parsers wired. 0.8 GPU mem (OOM-averse — DGX Spark UMA shares VRAM with desktop session + other workloads; bump to 0.85–0.9 with `-o gpu_memory_utilization=...` when running headless), 131k context, no prefix caching. `HF_HUB_OFFLINE=1` for offline parity after first cache fill. |
